...
The optimization variable is the state–input covariance generated by a causal
control policy. Because the set of achievable covariance matrices is only
implicitly defined, the method avoids projection onto this set. Instead, every
Frank–Wolfe linear minimization oracle is reduced to a finite-horizon
generalized LQ problem with an energy constraint. The oracle is solved using a
Riccati recursion and a one-dimensional search over a Lagrange multiplier.
