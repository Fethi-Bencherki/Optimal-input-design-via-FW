...
This repository contains the numerical experiments illustrating the paper’s Frank–Wolfe framework for optimal input design in finite-horizon linear systems. By replacing costly projections with linear minimization oracles, the approach exploits the structure of the input-design problem while retaining simple constrained updates. The examples include covariance design for known dynamics, experiment design with unknown dynamics, and adaptive online LQR balancing control performance and information acquisition.
